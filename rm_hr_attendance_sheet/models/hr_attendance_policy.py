...
class HrAttendancePolicy(models.Model):
    ...
    def get_overtime(self):
        print('over_time from fun')
        self.ensure_one()
        res = {}
        if self:
            overtime_ids = self.overtime_rule_ids
            print('over_time rule',overtime_ids)
            ...
            if wd_ot_id:
                print('work_day overtime',wd_ot_id)
                res['wd_rate'] = wd_ot_id.rate
                print('work_day overtime rate ',wd_ot_id.rate)
                res['wd_after'] = wd_ot_id.active_after
                print('work_day overtime active after',wd_ot_id.active_after)

            else:
                res['wd_rate'] = 1
                res['wd_after'] = 0
            if we_ot_id:
                print('week end overtime',we_ot_id)
    ...
    def get_late(self, period, cnt):
        ...
        if self.late_rule_id:
            print('self.late rule', self.late_rule_id)
