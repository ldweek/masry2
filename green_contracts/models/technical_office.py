...
class TechnicalOffice(models.Model):
    _name = 'technical.office'
    _order = 'id desc'
    ...
    distribution = fields.Selection([
        ...
        ('warehouse.department', 'Warehouse Dep'),
    ])
    ...
    def write(self, vals):
        if vals.get('distribution'):
            ...
            if model:
                record = self.env[model].search([('tech_office_line_id', '=', self.id)])
                if record:
                    raise ValidationError(
                        _(f'you cannot change the distribution where the {self.name} has already been distributed!\n\n'
                          f'If you wish to change it, you must first remove it from the {model}.'))
